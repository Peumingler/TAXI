...
=======
        `SELECT user_no, email, user_type, username, phone, rated_times, rate_average, active, create_time FROM users WHERE user_no = ${this.userId}`;
...
=======
        queryStr =
        `INSERT INTO users(email, password, password_salt, user_type, username, phone, rated_times, rate_average, active, create_time)
            VALUES('${email}', '${hash}', '${salt}', '${user_type}', '${nick}', '${phone}', 0, 0, false, now())`;
>>>>>>> 4eca2bbe1bf7dd9e7c9a103aa95b8a02978cf14f
...
=======
    ...
    let queryStr =
    `SELECT user_no, password, password_salt, active
    ...
    db.query(queryStr, (err, result) => {
        ...
        let userSalt = result[0]['password_salt'];
        let userHash = result[0]['password'];
